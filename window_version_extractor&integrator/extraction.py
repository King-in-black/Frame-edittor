...
def extract_frames(video_path, output_folder, fps, start_time, duration, process_number):
    ...
    ffmpeg_command = [
        ffmpeg_path, '-ss', str(start_time), '-t', str(duration), '-i', video_path, '-vf', f'fps={fps}',
        os.path.join(part_output_folder, f'{video_name}_frame_%07d.png')
    ]

    print(f"Running FFmpeg command: {' '.join(ffmpeg_command)}")  # 调试信息
...
def parallel_frame_extraction(video_path, output_folder, fps, num_processes):
    ...
    ffprobe_command = [
        ffprobe_path, '-v', 'error', '-select_streams', 'v:0', '-show_entries', 'format=duration', '-of',
    ...
    for i in range(num_processes):
        ...
        p = multiprocessing.Process(target=worker_function, args=(queue, video_path, output_folder, fps, start_time, chunk_duration, i))
        p.start()
        processes.append(p)
...
if __name__ == "__main__":
    ...
    tk.Button(root, text="Start Frame Extraction", command=start_frame_extraction).grid(row=3, column=0, columnspan=3, padx=10, pady=20)
